...
export default function EachProduct({ product, fetchData }) {
    ...
    const [dateValue, setDateValue] = useState(moment(product.date).format('YYYY-MM-DD'));

    const openModal = () => {
        document.getElementById('new-modal-' + product.id).classList.remove("hidden");
    }
    const closeModal = () => {
        document.getElementById('new-modal-' + product.id).classList.add("hidden");
    }
    const completeForm = () => {
        closeModal()
        fetchData()
    ...
    const updateUser = (e) => {
        e.preventDefault()
        const form = document.getElementById(`editform-${product.id}`);
        const formData = new FormData(form);
        axios.patch(`${API_URL}/products/${product.id}`, formData)
        ...
        <div className="bg-slate-100 rounded-lg mb-4 p-4 hover:border hover:border-purple-700">
            <div>
                <div>
                    ...
                    <div className="text-slate-400">{product.price}</div>
                </div>
            ...
            {/* Start Modal */}
            <div className="relative z-10 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true" id={`new-modal-${product.id}`}>
                ...
                <div className="fixed z-10 inset-0 overflow-y-auto">
                    <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                        ...
                        <div className="relative inline-block align-middle bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg w-full">
                            <form id={`editform-${product.id}`} onSubmit={updateUser} method="post">
